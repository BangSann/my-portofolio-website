...
const TechStack = () => {
  ...
  return (
    <section className="bg-slate-50">
      <section className="container px-2 mx-auto flex items-center justify-center relative h-[calc(100vh)] ">
        <div className="flex items-center gap-4 w-full">
          {/* LEFT: Title & Description */}
          <animated.div
            style={techStackAnimated}
            className="text-start h-full w-1/3"
          >
            <animated.h1
            ...
            <animated.p
              style={selectedTechAnimated}
              className="text-slate-600 text-justify font-light w-4/5"
            >
          ...
          <div className="grid grid-cols-2 gap-4 w-2/3">
            ...
            <div className="grid grid-cols-4 gap-4 justify-items-center">
              {techstackItems.map((item) => (
                ...
                >
                  <div
                    className={`shadow-sm rounded-lg w-full transition-transform duration-500 cursor-pointer 
                      ${
                  ...
                  >
                    <img
                      src={item.src}
                      alt={item.title}
                      className="w-15 h-15 object-contain m-2 inline-block"
                    />
                  </div>
                </animated.div>
              ))}
